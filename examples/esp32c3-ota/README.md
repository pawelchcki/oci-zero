...
### Flashing more than one board

The QR code is the same on every board, which is the point — but it has two
consequences. Discovery is ambiguous while more than one board is uncommissioned,
because the discriminator is the only filter a commissioner has when scanning BLE
and all of them advertise `3840`; commission one at a time, since a commissioned
node stops advertising. And the passcode is shared, so one leaked code covers
every board.

What does differ per board: the operational node ID and certificate assigned
during commissioning, and Basic Information's `UniqueID`, which is the hex of the
chip's factory MAC. Read `UniqueID` to tell two of these apart.

`SerialNumber` is deliberately *not* per-board. `rs-matter` folds a non-empty
serial number into the QR payload's optional TLV, so varying it would give every
board a different code and there would be nothing to commit here. Per-board QR
codes are the trade if you ever want per-board serials.
