...
fn configured_base_seed() -> u64 {
    match std::env::var("GZIP_ZERO_CROSSCHECK_SEED") {
        Ok(value) => value.parse::<u64>().unwrap_or_else(|error| {
            panic!("GZIP_ZERO_CROSSCHECK_SEED must be an unsigned 64-bit integer: {error}")
        }),
        Err(std::env::VarError::NotPresent) => BASE_SEED,
        Err(error) => panic!("cannot read GZIP_ZERO_CROSSCHECK_SEED: {error}"),
    }
}
